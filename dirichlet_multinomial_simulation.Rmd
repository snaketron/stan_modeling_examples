---
title: "Working with Dirichlet Multinomial data"
author: "SK"
date: "`r Sys.Date()`"
output:
...
u <- get(load(file = "gene_usage.RData"))
```
...
```{r, echo = FALSE}
save(fit_dm, file = "fit_dm.RData", compress = T)
```
...
```{r, echo = FALSE}
save(fit_m, file = "fit_m.RData")
```


```{r, echo=FALSE, eval = TRUE, echo=F}
fit_dm <- get(load(file = "fit_dm.RData"))
fit_m <- get(load(file = "fit_m.RData"))
```
...
```{r, eval=TRUE, echo = FALSE}
loglik_train_dm <- summary(fit_dm, par = "log_lik")$summary
...
```{r, eval=TRUE, echo = FALSE}
loglik_train_m <- summary(fit_m, par = "log_lik")$summary
