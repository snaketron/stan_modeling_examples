...
gamma_sigma = 0.001 # approx. complete pooling within gene
...
# What if we treated the DM distributed data as *unpaired*


```{r, echo=FALSE, eval=TRUE}
u <- get(load(file = "dm_sim/gene_usage_unpaired.RData"))
```
...
### STAN code for model $DM^u$ for **unpaired data**
...
### STAN code for model $M^u$ for **unpaired data**

```{stan, output.var = "model_m_u", class.source = 'fold-hide'}
data {
  int <lower = 0> N_sample; // number of samples (repertoires)
  int <lower = 0> N_gene; // number of genes
  int Y [N_gene, N_sample]; // number of successes (cells) in samples x gene
...
model {
  ...
  // likelihood
  for(i in 1:N_sample) {
    target += multinomial_lpmf(Y[,i]|softmax(alpha[i]+beta[i]*X[i]));
  }
}
...
generated quantities {
  ...
  for(i in 1:N_sample) {
    mu = softmax(alpha[i]+beta[i]*X[i]);
    Y_hat[,i] = multinomial_rng(mu, N[i]);
    log_lik[i] = multinomial_lpmf(Y[,i]|mu);
  }
  
  // PPC: condition-specific
  a = normal_rng(alpha_mu_gene, alpha_sigma_gene);
  b = normal_rng(beta_mu_gene, beta_sigma_gene);
  mu_group[1] = softmax(to_vector(a)+to_vector(b)*+1);
  mu_group[2] = softmax(to_vector(a)+to_vector(b)*-1);
  
  // PPC: train
  for(i in 1:N_sample) {
    if(X[i]==1) {
      log_lik_train[i] = multinomial_lpmf(Y[,i]|mu_group[1]);
    } 
    else {
      log_lik_train[i] = multinomial_lpmf(Y[,i]|mu_group[2]);
    }
  }
  ...
  for(i in 1:N_sample_test) {
    if(X_test[i]==1) {
      log_lik_test[i] = multinomial_lpmf(Y_test[,i]|mu_group[1]);
      Y_hat_group[,i] = multinomial_rng(mu_group[1], N[i]);
    }
    else {
      log_lik_test[i] = multinomial_lpmf(Y_test[,i]|mu_group[2]);
      Y_hat_group[,i] = multinomial_rng(mu_group[2], N[i]);
    }
  }
}
```
...
### Model fitting
We will fit models $DM^u$ and $M^u$ using $D_{\text{train}}$.
...
```{r, echo = FALSE}
save(fit_dm, file = "dm_sim/fit_dm_unpaired.RData", compress = T)
```


```{r}
fit_m <- rstan::sampling(object = model_m_u,
                         data = u,
                         chains = 3,
                         cores = 3,
                         iter = 6500,
...
```{r, echo = FALSE}
save(fit_m, file = "dm_sim/fit_m_unpaired.RData")
```


```{r, echo=FALSE, eval = TRUE, echo=F}
fit_dm <- get(load(file = "dm_sim/fit_dm_unpaired.RData"))
fit_m <- get(load(file = "dm_sim/fit_m_unpaired.RData"))
```
...
get_ppc_u <- function(fit, gene_usage, model_name) {
  s <- data.frame(summary(fit, par = "Y_hat")$summary)
  s$Y <- as.vector(t(gene_usage$Y))
  
  s$gene_name <- rep(x = gene_usage$gene_names, each = length(gene_usage$sample_names))
  s$sample_id <- rep(x = gene_usage$sample_names, times = length(gene_usage$gene_names))
  s$condition <- do.call(rbind, strsplit(x = s$sample_id, split = "_"))[,2]
...
```{r, eval = TRUE, echo = FALSE}
# DM
ppc_dm <- get_ppc_u(fit = fit_dm, gene_usage = u, model_name = "DM")

# M
ppc_m <- get_ppc_u(fit = fit_m, gene_usage = u, model_name = "M")
...
get_ppc_group <- function(fit, gene_usage, model_name) {
  ...
  s$Y_Dout <- as.vector(t(gene_usage$Y_test))
